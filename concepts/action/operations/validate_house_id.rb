...
class Action
  class ValidateHouseId < Trailblazer::Operation
    ...
    def model!(options, params:, **)
      begin
        response = RestClient.get("http://lb/houses/#{params[:house_id]}/actions/#{params[:id]}/validate", headers={'Authorization' => params[:authorization_header]})
    ...
    def log_success(options, params:, model:, **)
      LOGGER.info "[#{self.class}] Found valid action with params #{params.to_json}. House: #{options['model'].to_s}"
    end

    def log_failure(options, params:, **)
      LOGGER.info "[#{self.class}] Failed to find valid action with params #{params.to_json}"
    end
  end
end